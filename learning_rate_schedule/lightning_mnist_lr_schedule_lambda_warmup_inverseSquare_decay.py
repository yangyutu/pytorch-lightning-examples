...
class LitMNIST(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # This is from attention is all you need papaer: the linear warm up with following inverse square root decay
        optimizer = torch.optim.Adam(self.parameters(), lr=self.learning_rate)
        lr_warmup_steps = 2000

        def lr_foo(step):
            lr_scale = (lr_warmup_steps) ** 0.5 * min(
                (step + 1) ** (-0.5), step / lr_warmup_steps ** (1.5)
            )
...
def main():
    ...
    wandb_logger = WandbLogger(
        ...
        tags=["MNIST", "lr_scheduler_warmup_InverseSquareDecay"],
    )
